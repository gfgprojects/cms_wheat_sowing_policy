arrotondamento<-function(x){
	round(x,digits=5)
}

valutazione<-function(parameters){
print(paste(parameters[1],parameters[2],parameters[3],parameters[4]))
#========= update simulation inputs
tmp.production.data<-original.input.production.data
#modify Central Asia
tmp.production.data[3,29]<-round(tmp.production.data[3,29]*(1+parameters[1]))
#modify Pakistan
tmp.production.data[10,29]<-round(tmp.production.data[10,29]*(1+parameters[2])) 
#modify China
tmp.production.data[12,29]<-round(tmp.production.data[12,29]*(1+parameters[3])) 
write.table(tmp.production.data,"tmp_production_file.csv",sep=",",row.names=F,quote=F)
write("","tmp_production_file.csv",append=T)


tmp.demand.data<-original.input.demand.data
#modify Eastern Africa
tmp.demand.data[13,25]<-round(tmp.demand.data[13,25]*(1+parameters[4]))
write.table(tmp.demand.data,"tmp_demand_file.csv",sep=",",row.names=F,quote=F)
write("","tmp_demand_file.csv",append=T)

#=========== copia file di input della simulazione

system("cp tmp_production_file.csv ../rs_model/cms_wheat/data/producers.csv")
system("cp tmp_production_file.csv ../rs_model/cms_wheat/data/producers_w.csv")
system("cp tmp_demand_file.csv ../rs_model/cms_wheat/data/buyers_Misc.csv")
