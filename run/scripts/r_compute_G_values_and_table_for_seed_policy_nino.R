...
print(round(diff.quantities.changes[,-1],digits=2))

print("")
print("without Eastern Africa")

G_p_phase1<-sum(av.prices.change.nino[-1,]^2)
G_p_phase_plus_policy1<-sum(av.prices.change.nina[-1,]^2)
print(paste("G_p_phase",round(G_p_phase1,digits=2)))
print(paste("G_p_phase_plus_policy",round(G_p_phase_plus_policy1,digits=2)))
G_WA_phase1<-sum(imp.aggreg.change.nino.demand.ratio[-1,]^2)
G_WA_phase_plus_policy1<-sum(imp.aggreg.change.nina.demand.ratio[-1,]^2)
print(paste("G_WA_phase",round(G_WA_phase1,digits=3)))
print(paste("G_WA_phase_plus_policy",round(G_WA_phase_plus_policy1,digits=3)))
